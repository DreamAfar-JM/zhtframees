	...
	public List<?> findRoleCodeUserHaveForCombox(Long userId,Boolean isEnable){
	 ...
	 public List<String> findRoleCodeUserHave(String userName,Boolean isEnable){
		...
		public List<String> findRoleCodeFromUserExtendsDepartmentsByUserId(Long userId,	Boolean isEnable) {
			...
			List<Long> departmentIdList=sysDepartmentService.findDepartmentIdsByUserId(userId);
			if(departmentIdList==null||departmentIdList.size()==0){
				return null;
			}
			List<String> list=new ArrayList<String>();
			for(Long deptId : departmentIdList){
				List<String> grouHave=findRoleCodeSysDepartmentHave(deptId,isEnable);
				if(grouHave!=null){
					list.addAll(grouHave);
				}
			}
		...
		public DataSet findRoleDataSetFromUserExtendsDepartmentsByUserId(Long userId,Boolean isEnable) {
			if(userId==null){
			 ...
			 List<String> roleCodeListFromDepartments=findRoleCodeFromUserExtendsDepartmentsByUserId(userId,isEnable);
			 if(roleCodeListFromDepartments==null){
				 return null;
			 }
			 ParamObject po=new ParamObject(POType.H_O_C);
			  String hqlp="select "
						 ...
						 + "r.enabled as enabled, "
						 + "r.description as description "
						 + "@from RbacRole r where r.code in(:roleCodeListFromDepartments)";
			  if(isEnable!=null){
				    hqlp+=" and r.enabled=:isEnable ";
					po.addAllowNull("isEnable", isEnable);
			  }
			  po.addAllowNull("roleCodeListFromDepartments", roleCodeListFromDepartments);
			 DataSet dataSetp=baseDaoImpl.loadDataSet(hqlp,po);
		     return dataSetp;
		}
...
//--------------------------GROUP$S--------------------------------------------------------------------------------------------	
	...
	public List<String> findRoleCodeInSysDepartmentRole(Long sysDepartmentId,Boolean isEnable) {
		...
		ParamObject po=new ParamObject(POType.H_NO_NC);
		String hqlFromDepartmentRole=" select dr.rbacRole.code as code from SysDepartmentRole dr where  dr.sysDepartment.id=:sysDepartmentId ";
		if(isEnable!=null){
			hqlFromDepartmentRole+="and dr.rbacRole.enabled=:isEnable";
			po.addAllowNull("isEnable", isEnable);
		}
		po.addParam("sysDepartmentId", sysDepartmentId);
		@SuppressWarnings("unchecked")
		List<String> listFromDepartmentRole=(List<String>) baseDaoImpl.findJustList(hqlFromDepartmentRole,po);
		return listFromDepartmentRole;
	}
	@Override
	public DataSet findRoleDataSetInSysDepartmentRole(Long sysDepartmentId,Boolean isEnable) {
		if(sysDepartmentId==null){return null;}
		 ParamObject po= new ParamObject(POType.H_O_C);
			String hql="select "
					 + "dr.rbacRole.id as id, "
					 + "dr.rbacRole.code as code, "
					 + "dr.rbacRole.name as name, "
					 + "dr.rbacRole.enabled as enabled, "
					 + "dr.rbacRole.description as description "
					 + "@from SysDepartmentRole dr where dr.sysDepartment.id=:sysDepartmentId";
					if(isEnable!=null){
						hql+="and dr.rbacRole.enabled=:isEnable";
						po.addAllowNull("isEnable", isEnable);
					}
					po.addAllowNull("sysDepartmentId", sysDepartmentId);
			DataSet dataSet=baseDaoImpl.loadDataSet(hql,po);
			return dataSet; 
	}
		...
		if(sysDepartmentId==null){return null;}
		 ParamObject po= new ParamObject(POType.H_O_C);
			String hql=" select "
					 ...
					 + " r.description as description "
					 + " @from RbacRole r where "
					 + " r.id not in ( select sdr.rbacRole.id from SysDepartmentRole sdr where sdr.sysDepartment.id=:sysDepartmentId)";
					if(isEnable!=null){
						hql+="and r.enabled=:isEnable";
						po.addAllowNull("isEnable", isEnable);
					}
	...
	public List<String> findRoleCodeFromParentSysDepartments(Long sysDepartmentId, Boolean isEnable) {
		...
		List<String> allList=new ArrayList<String>();
		while(sysDepartmentId!=null){
			sysDepartmentId=(Long) baseDaoImpl.findParentIdBySelfUnique(RbacGroup.class, "parentSysDepartment", "id", sysDepartmentId);
			if(sysDepartmentId!=null){
				List<String> listFromSysDepartmentRole=findRoleCodeInSysDepartmentRole(sysDepartmentId, isEnable);
				if(listFromSysDepartmentRole!=null){
					allList.addAll(listFromSysDepartmentRole);
				}
			}
		}
		return allList;
	...
	public DataSet findRoleDataSetFromSysDepartmentParents(Long sysDepartmentId, Boolean isEnable) {
		 ...
		 List<String> roleCodelistFromParentSysDepartments=findRoleCodeFromParentSysDepartments(sysDepartmentId,isEnable);
		 if(roleCodelistFromParentSysDepartments==null||roleCodelistFromParentSysDepartments.size()==0){
			 return null;
		 }
		 ParamObject po= new ParamObject(POType.H_O_C);
		 String hqlp=  " select "
		 ...
		 po.addAllowNull("roleCodeList", roleCodelistFromParentSysDepartments);
	...
	public List<String> findRoleCodeSysDepartmentHave(Long sysDepartmentId,Boolean isEnable) {
		...
		// from  grouprole
		List<String> listFromDepartmentRole=findRoleCodeInSysDepartmentRole(sysDepartmentId, isEnable);
		// from parents
		List<String> listFromParents=findRoleCodeFromParentSysDepartments(sysDepartmentId,isEnable);
		
		List<String> list=new ArrayList<String>();
		if(listFromDepartmentRole!=null&&listFromDepartmentRole.size()>0){
			list.addAll(listFromDepartmentRole);
		}
		if(listFromParents!=null&&listFromParents.size()>0){
			list.addAll(listFromParents);
		}
	...
	public void removeRolesFromSysDepartmentRole(Long[] roleIds,Long sysDepartmentId) {
		if(sysDepartmentId==null||roleIds==null||roleIds.length==0){
			throw new ServiceLogicalException("请选择数据");
		}
		String hql="delete from SysDepartmentRole dr  where dr.sysDepartment.id=:sysDepartmentId and dr.rbacRole.id in (:roleIds)";
		baseDaoImpl.executeUpdate(hql, new ParamObject(POType.H_NO_NC).
				addAllowNull("sysDepartmentId", sysDepartmentId).
				addAllowNull("roleIds", roleIds));
	...
	public void addRolesToSysDepartmentRole(Long[] roleIds, Long sysDepartmentId) {
		...
		roleIds=(Long[]) ZBeanUtil.removeDuplicateWithOrder(roleIds);
		String hql="select dr.rbacRole.id from SysDepartmentRole dr where dr.sysDepartment.id=:sysDepartmentId";
		List<Long> roleIdInGroupRole=(List<Long>) baseDaoImpl.findJustList(hql, new ParamObject(POType.H_NO_NC).addAllowNull("sysDepartmentId", sysDepartmentId));
		if(roleIdInGroupRole==null){
			roleIdInGroupRole=new ArrayList<Long>();
		}
		List<SysDepartmentRole> drsList=new ArrayList<SysDepartmentRole>();
		for(Long roleId:roleIds){
			if(!roleIdInGroupRole.contains(roleId)){
				drsList.add(new SysDepartmentRole(new SysDepartment(sysDepartmentId),new RbacRole(roleId)));
			}
		}
		baseDaoImpl.saveOrUpdate(drsList);
	}
