...
//		String hqlFromRole=" select p.code from RbacPermission p join p.rbacRoles r where r.id = (select u.defaultRbacRole.id from RbacUser u where u.userName =:userName)";
		List<String> listFromRole=(List<String>) rbacPermissionDao.findJustList(hqlFromRole, new ParamObject(POType.H_NO_NC).addParam("userName", userName));
	...
	public void addPermissionsToRole(Long[] permIds, Long roleId) {
		...
		List<RbacRolePermission> rpsListToBeAdd=new ArrayList<RbacRolePermission>();
			for(Long permissionId:permIds){
				if(!permIdsInRP.contains(roleId)){
