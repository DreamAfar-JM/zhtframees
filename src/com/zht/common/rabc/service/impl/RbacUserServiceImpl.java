...
import com.zht.common.rabc.model.SysDepartmentUser;
...
public class RbacUserServiceImpl  extends BaseServiceImpl<RbacUser>implements IRbacUserService{
	...
	public DataSet loadRbacUserDataSet(ParamObject paramObject,RowMap rowMap) {
		return rbacUserDao.loadDataSetFromOneEntity(RbacUser.class, paramObject, rowMap);
	}
	@Override
...
//----------------------Department@S-----------------------------------------------------------------------	
	...
	public DataSet loadUserDataSetIsInSysDepartment(ParamObject paramObject,RowMap rowMap, Long departmentId, Boolean isIn) {
		...
		String extroConditon="";
    	if(isIn!=null&&isIn){
    		extroConditon=" and id in ( select gu.rbacUser.id from SysDepartmentUser gu where gu.sysDepartment.id="+departmentId+") ";
    	}else if(isIn!=null&&!isIn){
    		extroConditon=" and id not in ( select gu.rbacUser.id  from SysDepartmentUser gu where gu.sysDepartment.id="+departmentId+" ) ";
    	}
    	DataSet grid= baseDaoImpl.loadDataSetFromOneEntity(RbacUser.class,paramObject,rowMap, extroConditon);
		return grid;
	}
	...
	public void removeUserFromSysDepartment(Long[] userIds, Long departmentId) {
		...
		String hql=" delete from SysDepartmentUser where rbacUser.id in(:userIds) and sysDepartment.id=:departmentId";
	...
	public void addUserToSysDepartment(Long[] userIds, Long departmentId) {
		...
		String hql="select gu.rbacUser.id from SysDepartmentUser gu where gu.sysDepartment.id=:departmentId";
		...
		List<SysDepartmentUser> guListToBeAdd=new ArrayList<SysDepartmentUser>();
			for(Long userId:userIds){
				if(!userIdsInGU.contains(userId)){
					guListToBeAdd.add(new SysDepartmentUser(new RbacUser(userId),new SysDepartment(departmentId)));
				}
			}
		if(guListToBeAdd.size()>0){
			baseDaoImpl.saveOrUpdate(guListToBeAdd);
