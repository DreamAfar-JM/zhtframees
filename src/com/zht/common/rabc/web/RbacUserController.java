...
import org.zht.framework.filter.jcaptcha.JCaptchaFiltercc;
...
//-------------------------------CRUD--------------------------------------------------------------
    ...
    public Object loadUserGridView(@ModelAttribute("paramObject") ParamObject paramObject){
	    ...
	    DataSet grid= rbacUserService.loadRbacUserDataSet(paramObject, rowMap);
...
//--------------------------------USER-Permission ------Assign----------------------------------------------
    ...
    public Object login(HttpServletRequest request,String userName, String password,String jcaptchaCode) {
    	Boolean jcodeValidate=JCaptchaFiltercc.validateCaptchaChallenge(request, jcaptchaCode);
    	System.out.println(jcodeValidate);
    	  ...
    	  try {  
    		 ...
    		 // SpringUtils.getBean(ShiroDbRealm.class).clearCachedAuthorizationInfo(currentUser.getPrincipal().toString());  
